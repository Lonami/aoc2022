...
impl Play {
    fn score(&self) -> i32 {
        self.own.score() + self.own.beats(self.opponet).score()
    }
}
    ...
    Ok(input
        ...
        .try_fold(Vec::new(), |mut plays, line| {
            plays.push(Play {
                ...
                own: parse_hand(line.as_bytes()[2])?
            });
            Ok::<_, Error>(plays)
        })?)
}
...
fn main() -> Result<()> {
    ...
    println!("Part 1 answer: {}", get_plan_score(&input));
    Ok(())
}
...
mod tests {
    ...
    fn example1() -> Result<()> {
        ...
        assert_eq!(get_plan_score(&input), 15);
        Ok(())
    }
}
